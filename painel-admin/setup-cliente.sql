-- ==========================================================================
-- setup-cliente.sql — versão pra implantar num cliente novo
-- ==========================================================================
-- Cópia de setup.sql SEM a tabela do Radar de Notícias e sem o agendamento
-- (cron) dele — esses dois ficam só no seu projeto. Veja o Playbook do
-- Cliente Novo pro passo a passo completo (Fase B).
--
-- ANTES DE RODAR: troque 'TROQUE_PELO_UID_DA_CLIENTE' logo abaixo pelo User
-- UID da cliente (Authentication > Users, depois de criar o login dela).
-- ==========================================================================
...
stable
as $$
  select auth.uid() = 'TROQUE_PELO_UID_DA_CLIENTE'::uuid;
$$;
...
-- Tabela: planejador_cronograma_dias — qual pilar foi escolhido pra cada dia
-- específico do calendário. O cronograma semanal (pilar + dia_semana) acima
-- é só uma referência/sugestão; o calendário não se preenche sozinho a
-- partir dele — cada dia é escolhido na hora, direto no calendário.
create table if not exists public.planejador_cronograma_dias (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
  data date not null,
  pilar text not null
...
-- ---------------------------------------------------------------------
-- Tabela: clientes (controle de clientes do pacote painel+portfólio —
-- ver Playbook do Cliente Novo)
-- ---------------------------------------------------------------------
...
-- tabela; o agendamento do cron não entra aqui, cada cliente configura
-- separadamente quando for usar essa parte)
-- ---------------------------------------------------------------------
