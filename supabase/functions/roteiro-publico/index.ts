...
export default {
  fetch: withSupabase({ auth: "none" }, async (req, ctx) => {
    ...
    const { data: roteiro, error } = await ctx.supabaseAdmin
      .from("ugc_roteiros")
      .select("marca, produto, campanha, tipo_conteudo, duracao_prevista, objetivo, pontos_importantes, sugestao_legenda, marca_cor, moodboard_imagens, share_token, share_expira_em")
      .eq("id", id)
      .maybeSingle();
    ...
    const { data: cenas, error: erroCenas } = await ctx.supabaseAdmin
      ...
      .select("ordem, fala, o_que_fazer, tipo_cena, descricao_cena, faixa_tempo, funcao_cena, referencias_imagens")
      .eq("roteiro_id", id)
